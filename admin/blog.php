...
if (!function_exists('upload_image')) {
    function upload_image($file, $folder = 'blog') {
...
// Get categories for dropdown (blog categories)
$categories = [];
$result = mysqli_query($conn, "SELECT id, name FROM categories WHERE type = 'blog' ORDER BY name");
if ($result) {
...
if ($action === 'delete' && $id > 0) {
    // Get the blog post to delete its image
    $get_blog = mysqli_query($conn, "SELECT featured_image FROM blog_posts WHERE id = $id");
    if ($get_blog && mysqli_num_rows($get_blog) > 0) {
        $blog_data = mysqli_fetch_assoc($get_blog);
        
        // Delete the image file if exists
        if (!empty($blog_data['featured_image'])) {
            $image_path = __DIR__ . "/../uploads/blog/" . $blog_data['featured_image'];
    ...
    $result = mysqli_query($conn, "DELETE FROM blog_posts WHERE id = $id");
...
if ($action === 'toggle_featured' && $id > 0) {
    $result = mysqli_query($conn, "UPDATE blog_posts SET is_featured = NOT is_featured WHERE id = $id");
    if ($result) {
        $message = "Blog post featured status updated.";
    }
    $action = 'list';
}
...
if ($action === 'toggle_publish' && $id > 0) {
    $result = mysqli_query($conn, "UPDATE blog_posts SET is_published = NOT is_published WHERE id = $id");
    if ($result) {
        $message = "Blog post publish status updated.";
    }
    $action = 'list';
}
...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    if ($action === 'add' || $action === 'edit') {
        ...
        if ($action === 'add') {
            $check_slug = mysqli_query($conn, "SELECT id FROM blog_posts WHERE slug = '$slug'");
            if (mysqli_num_rows($check_slug) > 0) {
                $slug = $slug . '-' . time();
            }
        } else {
            // For edit, check if slug exists for other blog posts
            $check_slug = mysqli_query($conn, "SELECT id FROM blog_posts WHERE slug = '$slug' AND id != $id");
            if (mysqli_num_rows($check_slug) > 0) {
                $slug = $slug . '-' . time();
            }
        }
        ...
        if (isset($_FILES['featured_image']) && $_FILES['featured_image']['error'] === UPLOAD_ERR_OK) {
            ...
            if ($upload_result['success']) {
                ...
                if ($action === 'edit' && !empty($_POST['existing_image'])) {
                    $old_image = __DIR__ . "/../uploads/blog/" . $_POST['existing_image'];
        ...
        if (empty($title)) {
            ...
            if ($action === 'add') {
                ...
                $sql = "INSERT INTO blog_posts (
                    title, slug, category_id, tags, excerpt, content, 
                    featured_image, author, is_featured, is_published, 
                    published_at, created_at
                ) VALUES (
                    '$title', '$slug', $category_id, " . ($tags ? "'$tags'" : "NULL") . ",
                    " . ($excerpt ? "'$excerpt'" : "NULL") . ", " . ($content ? "'$content'" : "NULL") . ",
                    " . ($featured_image ? "'$featured_image'" : "NULL") . ", '$author',
...
if ($action === 'edit' && $id > 0) {
    ...
    $blog_item = mysqli_fetch_assoc($result);
...
<head>
    ...
    <title>Manage Blog Posts - TechInHausa Admin</title>
        ...
        <main class="main-content">
            ...
            <div class="top-bar">
                <div class="page-title">
                    ...
                    <p>
                        <?php if ($action === 'list'): ?>
                            <?php echo count($blog_items); ?> blog posts total
                        <?php else: ?>
                                ...
                                <tr>
                                    <td>
                                        <?php if (!empty($blog['featured_image'])): ?>
                                            <img src="<?php echo SITE_URL; ?>/uploads/blog/<?php echo htmlspecialchars($blog['featured_image']); ?>" 
                                        ...
                                        <?php else: ?>
                                            <div style="width: 80px; height: 60px; background: #f0f0f0; border-radius: 6px; display: flex; align-items: center; justify-content: center;">
                                                <i class="fas fa-blog" style="color: #ccc;"></i>
                                            </div>
                                        <?php endif; ?>
                                    </td>
                                    <td>
                                        ...
                                        <?php if (!empty($blog['tags'])): ?>
                                            <div class="blog-meta">
                                                <i class="fas fa-tags"></i> <?php echo htmlspecialchars($blog['tags']); ?>
                                            </div>
                                        <?php endif; ?>
                                    ...
                                    <td>
                                        <?php if (!empty($blog['category_name'])): ?>
                                            <span class="badge badge-info"><?php echo htmlspecialchars($blog['category_name']); ?></span>
                                        <?php else: ?>
                                    ...
                                    <td>
                                        <div><?php echo date('M j, Y', strtotime($blog['published_at'] ?? $blog['created_at'])); ?></div>
                                        <small style="color: #999;"><?php echo time_ago($blog['created_at']); ?></small>
                                    </td>
                                    <td>
                                        <div class="action-group">
                                            <a href="?action=edit&id=<?php echo $blog['id']; ?>" 
                                               class="action-icon" title="Edit">
                                                <i class="fas fa-edit"></i>
                                            </a>
                                            <a href="?action=toggle_featured&id=<?php echo $blog['id']; ?>" 
                                               class="action-icon" title="<?php echo $blog['is_featured'] ? 'Remove Featured' : 'Make Featured'; ?>">
                                                <i class="fas fa-star" style="color: <?php echo $blog['is_featured'] ? '#D3C9FE' : '#666'; ?>"></i>
                                            </a>
                                            <a href="?action=toggle_publish&id=<?php echo $blog['id']; ?>" 
                                               class="action-icon" title="<?php echo $blog['is_published'] ? 'Unpublish' : 'Publish'; ?>">
                                                <i class="fas <?php echo $blog['is_published'] ? 'fa-eye' : 'fa-eye-slash'; ?>"></i>
                                            </a>
                ...
                <div class="form-container">
                    <form method="POST" enctype="multipart/form-data">
                        <div class="form-grid">
                            ...
                            <div class="form-group full-width">
                                ...
                                <input type="text" name="title" class="form-control" 
                                       value="<?php echo htmlspecialchars($blog_item['title'] ?? ''); ?>" required>
                            </div>
                            ...
                            <div class="form-group">
                                ...
                                <input type="text" name="slug" class="form-control" 
                                       value="<?php echo htmlspecialchars($blog_item['slug'] ?? ''); ?>"
                                ...
                                <select name="category_id" class="form-control">
                                    ...
                                    <?php foreach ($categories as $cat): ?>
                                        <option value="<?php echo $cat['id']; ?>" 
                                            <?php echo (isset($blog_item['category_id']) && $blog_item['category_id'] == $cat['id']) ? 'selected' : ''; ?>>
                            ...
                            <div class="form-group">
                                ...
                                <input type="text" name="author" class="form-control" 
                                       value="<?php echo htmlspecialchars($blog_item['author'] ?? 'MalamIromba'); ?>">
                            </div>
                            ...
                            <div class="form-group full-width">
                                ...
                                <input type="text" name="tags" class="form-control" 
                                       value="<?php echo htmlspecialchars($blog_item['tags'] ?? ''); ?>"
                            ...
                            <div class="form-group full-width">
                                ...
                                <?php if (!empty($blog_item['featured_image'])): ?>
                                    <input type="hidden" name="existing_image" value="<?php echo htmlspecialchars($blog_item['featured_image']); ?>">
                                    <div class="current-image">
                                        <img src="<?php echo SITE_URL; ?>/uploads/blog/<?php echo htmlspecialchars($blog_item['featured_image']); ?>" alt="Current image">
                                        <span>Current image: <?php echo htmlspecialchars($blog_item['featured_image']); ?></span>
                                    </div>
                                <?php endif; ?>
                                <small style="color: #666;">Recommended size: 1200x630px</small>
                            </div>
                            
                            <!-- Excerpt -->
                            <div class="form-group full-width">
                                <label><i class="fas fa-paragraph"></i> Excerpt (Short summary)</label>
                                <textarea name="excerpt" class="form-control" rows="3"><?php echo htmlspecialchars($blog_item['excerpt'] ?? ''); ?></textarea>
                                <small style="color: #666;">A brief summary of the blog post (optional)</small>
                            </div>
                            
                            <!-- Content -->
                            <div class="form-group full-width">
                                <label><i class="fas fa-align-left"></i> Content *</label>
                                <textarea name="content" class="form-control" rows="10" required><?php echo htmlspecialchars($blog_item['content'] ?? ''); ?></textarea>
                            </div>
                            ...
                            <div class="form-group">
                                ...
                                <input type="datetime-local" name="published_at" class="form-control" 
                                       value="<?php echo isset($blog_item['published_at']) ? date('Y-m-d\TH:i', strtotime($blog_item['published_at'])) : ''; ?>">
                            </div>
                            ...
                            <div class="form-group">
                                ...
                                <div class="form-check">
                                    <input type="checkbox" name="is_featured" id="is_featured" value="1"
                                        <?php echo (isset($blog_item['is_featured']) && $blog_item['is_featured']) ? 'checked' : ''; ?>>
                                    <label for="is_featured">Feature this post</label>
                                </div>
                                <div class="form-check">
                                    <input type="checkbox" name="is_published" id="is_published" value="1"
                                        <?php echo (!isset($blog_item['is_published']) || $blog_item['is_published']) ? 'checked' : ''; ?>>
                        ...
                        <div class="form-actions">
                            <a href="blog.php" class="btn btn-warning">Cancel</a>
                            <button type="submit" class="btn btn-primary">
                                <i class="fas fa-save"></i> 
                                <?php echo $action === 'add' ? 'Save Blog Post' : 'Update Blog Post'; ?>
                            </button>
                        </div>
                    </form>
                </div>
